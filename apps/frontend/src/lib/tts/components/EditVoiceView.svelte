...
<div class="flex h-full flex-col">
	...
	{#if cloneSettingsMessage}
		<div class="mb-4 rounded-md border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-400">
			{cloneSettingsMessage}
		</div>
	{/if}
	...
	<div class="flex-1 space-y-4 overflow-y-auto pb-4">
		...
		{#if selectedCloneSetting?.ref_audio_path}
			<div class="space-y-1.5">
				...
				<audio
					controls
					src={selectedCloneSetting.ref_audio_path}
					class="w-full"
					preload="none"
				></audio>
			</div>
		...
		{#if selectedCloneSetting?.ref_text}
			<div class="space-y-1.5">
				...
				<p class="text-muted-foreground rounded-md border border-dashed border-zinc-700 bg-zinc-800/30 px-3 py-2 text-xs italic">
