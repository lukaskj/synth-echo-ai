<script lang="ts">
	...
	let shouldShowSpeechRecognitionHint = $derived(!isSpeechRecognitionSupported && !hasReferenceTranscript);
...
<div class="space-y-4 rounded-lg border border-slate-800 bg-slate-950/50 p-4">
	<div class="flex flex-wrap items-start justify-between gap-3">
		...
		<div class="flex flex-wrap gap-2">
			{#if cloneView !== 'create'}
				<Button type="button" variant="secondary" size="small" onclick={onRefreshCloneSettings} disabled={isCloneSettingsLoading}>
	...
	{#if cloneSettingsErrorMessage}
		<NoticeBanner tone="error" className="p-4" role="alert">{cloneSettingsErrorMessage}</NoticeBanner>
