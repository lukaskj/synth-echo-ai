<script lang="ts">
	...
	const buttonLabel = $derived(
		status === 'synthesizing' ? UI_TEXT.synthesizing :
		status === 'cloning' ? UI_TEXT.cloning :
		status === 'loading-model' ? 'Loading model...' :
		isCloneMode ? UI_TEXT.cloneVoice : UI_TEXT.generateSpeech
	);
</script>
...
<Card.Root class="flex flex-col">
	<Card.Header class="pb-3">
		<div class="flex items-center justify-between gap-3">
			...
			<!-- Mode selector -->
		<Select.Root value={mode} onValueChange={(v) => { mode = v as TtsMode; }}>
